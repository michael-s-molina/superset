 ...
 * fifth — on one dashboard, the same "EMEA" in three blocks in three colours.
...
export function applySeriesDefaults(option: Dict, theme: ChartTheme): Dict {
  ...
  if (Array.isArray(series)) {
    return {
      ...
      series: series.map(item =>
        isDict(item)
          ? colorByName(
              fillAbsent(item, seriesDefaults(item.type as string, theme)) as Dict,
              theme,
            )
          : item,
      ),
  ...
  if (isDict(series)) {
    return {
      ...option,
      series: colorByName(
        fillAbsent(series, seriesDefaults(series.type as string, theme)) as Dict,
        theme,
      ),
    };
  }
