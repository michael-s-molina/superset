 ...
 *
 * Blocks draw with whatever renderer they like — ECharts, Vega-Lite, plain SVG —
 ...
 *
 * The fix is that theme compatibility should be the default a block starts from
 * rather than something its author remembers to implement. So the host states
 * the theme once, semantically, and each renderer maps those few fields onto its
 * own config — a mapping that is a handful of lines, unlike re-deriving what a
 * theme means. A block that genuinely wants different colours still says so: its
