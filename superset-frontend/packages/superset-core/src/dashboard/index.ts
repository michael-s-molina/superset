 ...
 *
 * A building block that draws its own chart has to colour its series itself,
 ...
 * charting library.
 *
 * A block draws with whatever renderer it likes, and every charting library
 * ships its own palette and its own near-black text — so without this, a block
 * looks like its library rather than like Superset, and two blocks on one
 * dashboard disagree about what "the first series" is. Map these few fields onto
 * your renderer's own config and merge your spec *over* the result, so a block
 ...
 * where it sits. See the field's own note for why that matters across blocks.
 *
...
export interface ChartTheme {
   ...
   * read across its blocks, and by position "EMEA" is the second colour in a
   * chart that happens to list it second and the fifth in one that does not —
   * so the same thing is a different colour in every block it appears in.
 ...
 * the light/dark theme can change while a block is mounted.
 */
